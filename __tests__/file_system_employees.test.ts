...
import {FileSystemEmployees} from "../lib/file_system_employees";
import {date} from "../lib/date_functions";

test('Reads a single Employee from the file system', () => {
    const actual = new FileSystemEmployees("__tests__/test_single_employee.csv").all();
...
test('Reads multiple Employees from the file system', () => {
    const actual = new FileSystemEmployees("__tests__/test_multiple_employees.csv").all();
...
test('Throws an exception on construction if the filename does not exist', () => {
    expect(() => new FileSystemEmployees("__tests__/this_file_does_not_exist.txt")).toThrow(/no such file/i);
});
